...
def test_scan_input_still_returns_existing_keys():
    """Regression guard: 0c955fb extended scan_input's return dict with
    `redacted_text`; existing keys (`safe`, `flags`, `risk_score`, `details`)
    must still be present so older consumers don't break."""
    result = scan_input("plain hello world with no identifiers")

    for key in ("safe", "flags", "risk_score", "details", "redacted_text"):
        assert key in result, f"scan_input missing key {key!r}"
    assert isinstance(result["flags"], list)
    assert isinstance(result["details"], dict)
    assert isinstance(result["risk_score"], int)


def test_scan_output_still_returns_existing_keys():
    """Regression guard: 0c955fb extended scan_output's return dict with
    `redacted_text` + `redactions`; `safe`, `flags`, `pii_detected` must
    still be present."""
    result = scan_output("All clear, nothing sensitive here.")

    for key in ("safe", "flags", "pii_detected", "redacted_text", "redactions"):
        assert key in result, f"scan_output missing key {key!r}"
    assert isinstance(result["flags"], list)
